...
const content = {
  zh: {
    ...
    maintenancePlans: [
      ...
      { name: "主機與系統代管", price: "NT$ 300–500 / 月起", subtitle: "適合需要系統長期穩定運行的專案", items: ["Render / Vercel / 資料庫代管", "基本部署與環境檢查", "主機費由實際平台收取", "不包含在一次性開發費內"] },
    ],
    ...
    estimateOptions: [
      ...
      ["data-query", "資料查詢功能", 9000, "讓使用者能查詢名單、資料清單、活動資訊或服務內容"],
      ["location", "定位搜尋 / 據點推薦", 12000, "依使用者位置回傳附近據點、導航連結或推薦結果"],
      ["admin-basic", "基礎後台資料管理", 15000, "管理者可新增、修改、刪除資料，不必每次找工程師"],
      ["admin-advanced", "進階後台 / 權限管理", 12000, "登入、角色權限、多資料表或更完整管理流程"],
      ["dashboard", "Dashboard / 使用紀錄", 12000, "紀錄查詢、點擊、熱門需求，提供成果追蹤"],
      ["ranking", "排序模型 / NTS 或 Trust Score 類功能", 18000, "需要更精準推薦、版本比較或資料可信度排序時使用"],
    ],
    clearAll: "一鍵清除",
    otherCustom: "其他客製功能",
    ...
    chooseFirst: "請先勾選功能",
    estimateDesc: "估價會把左側勾選的功能模組費用加總，再抓一個約 20% 的彈性範圍。這裡顯示的是整個專案的粗估區間，不是單一功能的獨立成案價格。正式報價仍需確認資料量、畫面數、流程細節、驗收範圍與維護需求。",
    subtotal: "功能模組加總",
    ...
    selectedTitle: "已選功能",
    noneSelected: "尚未選擇功能。",
    reminder: "提醒：這裡比較像功能配件表，用來抓預算量級；不是每個功能獨立成案的價格。正式成案仍需把功能範圍、資料量、交付內容、測試期與後續維護一起確認。若系統需要穩定長期運行，主機費通常至少約 US$7 / 月起，屬於每月固定成本，與一次性開發費分開計算。",
  ...
  en: {
    ...
    estimateOptions: [
      ...
      ["data-query", "Data search feature", 9000, "Let users search lists, data, events, or service content"],
      ["location", "Location search / service point recommendation", 12000, "Return nearby points, navigation links, or recommendations from user location"],
      ...
      ["dashboard", "Dashboard / usage logs", 12000, "Log queries, clicks, and popular needs for outcome tracking"],
      ["ranking", "Ranking model / NTS or Trust Score-like feature", 18000, "For more precise recommendation, version comparison, or data reliability ranking"],
    ],
    clearAll: "Clear all",
    ...
    estimateDesc: "The estimate adds selected module costs and gives a 20% flexible range. It is a rough project range, not a standalone quote for a single feature. Final pricing depends on data volume, screens, flow details, acceptance scope, and maintenance needs.",
