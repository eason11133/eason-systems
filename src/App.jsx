...
const content = {
  zh: {
    ...
    heroText:
      "如果你現在用 Google 表單、Excel、LINE、Email、社群貼文一起處理活動或課程，很容易遇到：一直回重複問題、名單狀態不清楚、報名後通知很亂、資料整理很花時間。我可以幫你把這些流程整理成網站、LINE Bot、後台管理或內部 CRM，讓使用者查得到資訊，管理者也不用每天手動救火。",
    ...
    rightSmall: "我實際可以幫你做什麼",
    rightTitle: "從表單、LINE、Excel 變成一套流程",
    rightCards: [
      ["活動 / 課程資訊頁", "把活動介紹、報名連結、時間地點、注意事項、FAQ 與聯絡方式整理成清楚頁面。"],
      ["LINE Bot / FAQ 分流", "讓使用者在 LINE 查活動資訊、報名問題、交通方式、聯絡窗口，不用每個問題都人工回。"],
      ["後台 / CRM / Dashboard", "讓管理者可以更新資料、整理名單狀態、追蹤進度，之後也能看查詢、點擊或成果統計。"],
    ],
    miniStats: [["網站", "資訊頁"], ["LINE", "查詢/FAQ"], ["後台", "管理/統計"]],
    problemLabel: "Problem",
    problemTitle: "你缺的通常不是漂亮頁面，而是少掉一套能省人工的流程",
    painPoints: [
      "報名表在 Google，FAQ 在 LINE，通知靠人工複製貼上",
      "報名後誰要通知、誰已回覆、誰還沒處理，很難追蹤",
      "資料散在 Excel、網站、PDF、Google 表單、社群貼文與 LINE 對話中",
      "LINE 官方帳號不只是公告欄，而是想變成查詢、分流與客服入口",
      "每次改資訊、回問題、整理名單都靠人工，越辦活動越混亂",
      "希望留下查詢、點擊、報名狀態或活動成果數據，而不是活動結束後再手動整理",
    ],
    googleLabel: "Google Forms vs System",
    googleTitle: "不是取代 Google，而是幫你整合 Google 做不到的部分",
    googleText: "如果只是單純報名表，我不建議你花錢做系統；Google 表單、試算表和現成工具就很好用。但如果你遇到的是報名後通知、FAQ、名單狀態、LINE 回覆、資料整理與成果統計分散在不同地方，我可以幫你把它們串成一套流程。",
    googlePoints: [
    ...
    audienceTitle: "優先服務有活動、有收入、有流程痛點但系統還沒成熟的團隊",
    audienceText: "客群會依照預算與流程痛點排序：第一優先是活動公司、才藝班、課程品牌、營隊、親子活動、地方創生、社會企業與教育新創；第二優先是有勸募、活動或課程的財團法人與 NGO；第三優先才是小型社團法人或小型協會。小店與個人品牌只保留為小型入門合作。",
    audienceGroups: [
      ["第一優先｜活動公司 / 課程品牌 / 營隊 / 親子活動", "有報名、有收入、有客服、有名單管理需求，最適合從流程系統切入"],
      ["第一優先｜地方創生 / 社會企業 / 教育新創", "常有活動、課程、據點、成果統計與對外服務流程，但系統通常還沒完全成熟"],
      ["第二優先｜財團法人公益組織 / 有活動課程的 NGO", "有預算與正式流程，適合談中型系統，但需要避開已經成熟的既有系統"],
      ["第三優先｜小型社團法人 / 小型協會", "可能有痛點但預算較低，適合小方案或先做流程評估，不作為主力開發對象"],
    ],
    ...
    buildTitle: "把活動、LINE、Google 表單、資料與後台整理成同一套流程",
    buildText:
      "適合不是只缺一張表單，而是報名、FAQ、LINE 詢問、名單狀態、資料更新與成果統計分散在不同工具的團隊。第一步會先確認哪些繼續用 Google 工具就好，哪些需要我幫你串成查詢、通知、後台或 Dashboard。",
    features: [
      { icon: MapPin, title: "活動 / 課程資訊頁", text: "整理活動介紹、報名連結、時間地點、注意事項、FAQ、交通方式與聯絡入口。" },
      { icon: Bot, title: "LINE Bot / FAQ 查詢", text: "把重複詢問變成按鈕與自動回覆，讓使用者可以自己查資訊、表單、據點或聯絡窗口。" },
      { icon: Database, title: "後台資料管理", text: "讓你可以自己更新活動、FAQ、據點、名單或連結，不用每次改內容都找工程師。" },
      { icon: LineChart, title: "Dashboard / CRM", text: "整理名單狀態、追蹤日、收款、交付日期，或留下查詢、點擊、報名與活動成果數據。" },
    ],
    flowLabel: "Service Flow",
    flowTitle: "合作會先從「你現在哪裡最亂」開始",
    offerSteps: [
      ["01｜先判斷適不適合做系統", "如果只是收報名，我會建議先用 Google 表單；如果卡在通知、查詢、狀態、資料管理，才適合做系統。"],
      ["02｜整理第一版流程", "把使用者會怎麼查、怎麼報名、怎麼收到資訊，以及管理者要怎麼更新資料先畫清楚。"],
      ["03｜做出可上線版本", "依需求做成活動頁、LINE Bot、查詢流程、後台管理、Dashboard 或 CRM，讓它真的能用。"],
      ["04｜再決定要不要擴充", "上線後再看是否需要登入、權限、報表、月維護、更多自動化或第二階段功能。"],
    ],
    ...
    contactTitle: "你不需要先決定要做什麼系統，只要先告訴我哪一段流程最讓你人工救火",
    contactText: "如果你現在只是覺得報名、通知、FAQ、LINE 回覆、資料整理或成果統計很亂，但還不確定該不該做系統，可以先做一次流程評估。我會協助判斷哪些用 Google 表單就夠、哪些值得串成系統、哪些功能可以留到第二階段。",
  ...
  en: {
    navContact: "Contact",
    badge: "Mid-sized workflow systems for teams with real operational pain",
    heroTop: "Not just a website —",
    heroHighlight: "a workflow that reduces manual firefighting",
    heroText:
      "I help event companies, course brands, camps, local revitalization teams, social enterprises, education startups, and organizations with recurring activities connect registration, FAQs, LINE inquiries, status tracking, data management, and outcome reports into one practical workflow. The point is not to replace Google Forms, but to integrate the parts Google tools do not solve well.",
    ...
    rightTitle: "Messy workflow → usable system",
    rightCards: [
      ["Decide whether a system is worth building", "If a simple Google Form is enough, I will say so. A custom system only makes sense when follow-ups, status tracking, LINE replies, and admin work become messy."],
      ["Turn manual firefighting into a process", "Connect registration follow-ups, FAQs, lists, LINE replies, admin updates, and outcome data into a repeatable workflow."],
    ...
    audienceLabel: "Who It Fits",
    audienceTitle: "Best for teams with activities, revenue, workflow pain, and immature systems",
    audienceText: "The priority is adjusted by budget and workflow pain: first, event companies, talent classes, course brands, camps, parent-child activities, local revitalization teams, social enterprises, and education startups; second, foundations or NGOs with fundraising, events, or courses; third, small associations with limited budget. Shops and personal brands remain starter-plan options only.",
    audienceGroups: [
      ["Priority 1｜Event companies / course brands / camps", "Teams with registration, revenue, customer service, and list-management needs are the best fit for workflow systems."],
      ["Priority 1｜Local revitalization / social enterprises / education startups", "They often have events, courses, service points, outcome reports, and public-facing workflows, but their systems are not mature yet."],
      ["Priority 2｜Foundations / NGOs with events or courses", "They may have budget and formal workflows, but the key is finding areas not already covered by existing systems."],
      ["Priority 3｜Small associations", "They may have pain but lower budget, so they are better suited for starter plans or a process check first."],
    ],
    eventSystemLabel: "Event / Forum Systems",
    ...
    buildLabel: "What I Build",
    buildTitle: "What I can help build usually falls into four types",
    buildText:
